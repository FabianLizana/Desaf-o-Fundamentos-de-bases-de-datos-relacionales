...
SELECT r.nombre_actor
FROM peliculas p INNER JOIN reparto r ON p.id = r.id_pelicula
WHERE p."Pelicula" = 'Titanic';

--5. Consultar en cuántas películas del top 100 participa Harrison Ford.
SELECT COUNT(p."Pelicula")
FROM peliculas p INNER JOIN reparto r ON p.id = r.id_pelicula
WHERE LOWER(r.nombre_actor) = 'harrison ford';
...
SELECT p."Pelicula", LENGTH(p."Pelicula") AS longitud_titulo
FROM peliculas p
WHERE LENGTH(p."Pelicula") = (SELECT MAX( LENGTH(p."Pelicula") )
FROM peliculas p );